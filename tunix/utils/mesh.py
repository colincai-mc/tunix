...
def group_devices_by_host(devices: Sequence[Any]) -> list[list[Any]] | None:
  ...
  Notes:
    ...
    inside each bucket are sorted with `allocation_device_sort_key()` so later
    whole-host allocation is reproducible.
  """
  host_to_devices = {}
  for device in devices:
    host_key = device_host_key(device)
...
def _find_best_region_candidate(
    coord_topology: CoordTopology,
    region: CoordRegion,
    candidate_shapes: Sequence[tuple[int, ...]],
) -> tuple[tuple[int, ...], tuple[int, ...], list[tuple[int, ...]]] | None:
  """Finds the best valid candidate box that fits within one coord region.

  Args:
    coord_topology: Normalized coord metadata for the device pool.
    region: Remaining region currently under consideration.
    candidate_shapes: Supported shapes that may be carved from ``region``.

  Returns:
    ``(start, shape, coords)`` for the best box in the region, or ``None`` if
    no valid box exists.
  """
  best_candidate = None
  best_score = None
  for shape in candidate_shapes:
    for start in sorted(coord_topology.coord_to_device):
      if not _region_contains_box(region, start, shape):
        continue
...
def find_best_candidate_coords(
  ...
  """Finds the best candidate box without materializing every candidate.
  ...
  Returns:
    The coord list for the best-ranked candidate box, or None when no valid box
    exists.

  Notes:
    This is the allocation-time fast path. It streams through candidate starts
    and keeps only the current best result.
  """
  shapes = candidate_shapes or _enumerate_box_shapes(
      required_devices,
      coord_topology.max_shape,
  )
  starts = sorted(coord_topology.coord_to_device)
...
def _allocate_devices_by_coords(
  ...
  Notes:
    ...
    3. Stream candidate boxes and keep only the best-scoring one.
  """
  coord_topology = get_coord_topology(devices)
  if coord_topology is None:
    return None, None
  ...
  for region, candidate_shapes in candidate_regions:
    best_region_candidate = _find_best_region_candidate(
        coord_topology,
        region,
        candidate_shapes,
    )
...
def _create_device_allocation_state(
  ...
  if host_groups:
    if host_bound_shape is None:
      raise ValueError(
          "Host-group allocation requires a known host-bound shape."
      )
    for host_devices in host_groups:
      if not _satisfies_host_bound_shape(
          host_devices,
...
def _allocate_devices_from_pool(
  ...
  The current preference order inside one pool is:

  1. Coord-box allocation when usable coords are available.
  2. Whole-host allocation when host grouping is available.
  3. Flat prefix allocation as the final fallback.

  Args:
    required_devices: Number of devices requested.
    remaining_devices: Current flat device pool.
    full_devices_per_host: Full logical device count for one host.
    host_bound_shape: Expected per-host topology shape.
  ...
  host_groups = group_devices_by_host(remaining_devices)
  if host_groups:
    assigned_devices = _allocate_from_host_groups(
        host_groups,
        required_devices,
        full_devices_per_host,
        host_bound_shape,
    ...
    return assigned_devices, remaining_devices, _create_coord_regions(remaining_devices)
...
def allocate_devices(
  ...
  2. Try pool-local allocation, which prefers coord-box placement and falls
     back to whole-host allocation when coord placement is unavailable.
  3. When slice metadata is available and no allocation has been made yet,
     prefer satisfying the request within one slice.
  4. If a request must span slices, only whole remaining slices may be used.
     Slices that were already partially consumed are skipped.
  5. If nothing above assigned devices, retry the pool-local allocator on the
     remaining pool.
  ...
  if assigned_devices is None and slice_groups and len(slice_groups) > 1:
  ...
  # If slice-aware policy did not assign anything, fall back to pool-local
  # allocation on the remaining devices.
  if assigned_devices is None:
    pool_slice_id = None
    if slice_groups and len(slice_groups) == 1:
      pool_slice_id = device_slice_id(slice_groups[0][0])
    pool_coord_regions = None
    if remaining_coord_regions_by_slice is not None:
      pool_coord_regions = remaining_coord_regions_by_slice.get(pool_slice_id)
    assigned_devices, remaining_devices, next_pool_coord_regions = _allocate_devices_from_pool(
        required_devices,
        remaining_devices,
        state.full_devices_per_host,
        state.host_bound_shape,
        mesh_name,
        coord_regions=pool_coord_regions,
      allocation_policy=state.allocation_policy,
    )
    if remaining_coord_regions_by_slice is not None:
      remaining_coord_regions_by_slice[pool_slice_id] = next_pool_coord_regions or ()
...
def _allocate_from_host_groups(
  ...
  """Allocates from remaining per-host buckets using whole hosts only.

  Args:
    host_groups: Deterministically ordered per-host device buckets.
    required_devices: Number of devices needed for the mesh.
    full_devices_per_host: Expected full logical-device count per host.
    host_bound_shape: Expected host topology shape used earlier for validation.
    mesh_name: Mesh name used for diagnostics.

  Returns:
    A flat device list consisting of the first `required_hosts` whole host
    buckets.

  Raises:
    ValueError: If the request would require partial hosts or more hosts than
      remain available.
  """
  if host_bound_shape is None:
    raise ValueError(
  ...
  if required_devices % full_devices_per_host != 0:
    raise ValueError(
        f"Mesh allocation for {mesh_name} requires {required_devices} devices, "
        "but whole-host allocation does not support partial hosts."
    )

  remaining_host_groups = [list(group) for group in host_groups]
  required_hosts = required_devices // full_devices_per_host
  if required_hosts > len(remaining_host_groups):
    raise ValueError(
        f"Mesh allocation requires {required_hosts} hosts for {mesh_name}, "
        f"but only {len(remaining_host_groups)} are available."
    )

  selected_host_groups = remaining_host_groups[:required_hosts]
  assigned_devices = [
      device for host_devices in selected_host_groups for device in host_devices
  ]
  return assigned_devices
