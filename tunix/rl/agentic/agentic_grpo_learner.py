...
class GRPOLearner(agentic_rl_learner.AgenticRLLearner[TGrpoConfig]):
  ...
  def _process_results(
    ...
    # Preserve a snapshot of the assistant-vs-env mask before degenerate-group
    # filtering zeroes ``completion_mask``. Used downstream by the
    # sampler-vs-trainer logp/prob diagnostic so the metric still has tokens
    # to measure when an entire group has uniform-zero advantages.
    diag_completion_mask = completion_mask
    ...
    # consistent log-probabilities.
    if (
        rollout_per_token_logps is not None
        and trainer_per_token_logps is not None
    ):
      # Mask selection: prefer the assistant-vs-env mask, but fall back to
      # positions where the rollout sampler actually returned a non-zero
      # logprob if the assistant mask happens to be empty (which occurs when
      # ``step.assistant_tokens`` is not populated upstream — e.g. when the
      # rollout produces zero captured assistant tokens for every trajectory
      # in a batch). Pad / env positions always have rollout logp == 0, while
      # tokens generated by the policy have a real non-zero logp, so the
      # fallback is in practice equivalent to the assistant mask but does
      # not depend on the trajectory engine populating it.
      asst_mask = diag_completion_mask.astype(jnp.bool_)
      sampler_mask = rollout_per_token_logps != 0.0
      mask = jnp.where(
          asst_mask.any(), asst_mask, sampler_mask
      )
      ...
      logging.info(
          ...
          " pearson=%.5f n_masked=%d (asst_mask_used=%s)",
          diff_mean, diff_max, prob_diff_mean, prob_diff_max, pearson,
          int(mask_f.sum()), bool(asst_mask.any()),
      )
