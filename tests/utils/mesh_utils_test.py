...
class MeshUtilsTest(absltest.TestCase):
  ...
  def test_allocate_named_mesh_device_slices_uses_task_id_host_groups(self):
    class FakeDevice:
  ...
  def test_allocate_devices_prefers_single_slice_before_host_fallback(self):
    class FakeDevice:
  ...
  def test_allocate_devices_skips_partial_slice_for_cross_slice_request(self):
    ...
    fake_devices = [
        ...
        FakeDevice(9, 2, (10, 0, 0)),
        FakeDevice(10, 2, (12, 0, 0)),
        FakeDevice(11, 2, (14, 0, 0)),
    ]
  ...
  def test_allocate_named_mesh_device_slices_uses_whole_hosts(self):
    ...
    self.assertEqual([device.id for device in allocated["trainer"]], [0, 2, 1, 3])
    self.assertEqual([device.id for device in allocated["rollout"]], [4, 6, 5, 7])

  def test_allocate_named_mesh_device_slices_raises_on_partial_host_request(self):
    class FakeDevice:
    ...
    with self.assertRaisesRegex(ValueError, "does not support partial hosts"):
      with mock.patch.object(mesh, "_allocate_devices_by_coords", return_value=(None, None)):
  ...
  def test_allocate_named_mesh_device_slices_orders_hosts_by_slice_then_host(self):
    class FakeDevice:
    ...
    fake_devices = [
    FakeDevice(7, 1, 1, (1, 1, 1)),
    FakeDevice(5, 1, 1, (1, 0, 1)),
    FakeDevice(3, 0, 0, (1, 1, 0)),
    FakeDevice(1, 0, 0, (1, 0, 0)),
    FakeDevice(6, 1, 1, (0, 1, 1)),
    FakeDevice(4, 1, 1, (0, 0, 1)),
    FakeDevice(2, 0, 0, (0, 1, 0)),
    FakeDevice(0, 0, 0, (0, 0, 0)),
    ]
    ...
    self.assertEqual([device.id for device in allocated["mesh1"]], [0, 2, 1, 3])
    self.assertEqual([device.id for device in allocated["mesh2"]], [4, 6, 5, 7])

  def test_allocate_named_mesh_device_slices_raises_on_non_multiple_host_request(self):
    class FakeDevice:
  ...
  def test_allocate_named_mesh_device_slices_raises_on_cross_host_remainder(self):
    class FakeDevice:
